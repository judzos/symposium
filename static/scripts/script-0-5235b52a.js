(function progressRuntime() {
  ...
  function renderBand(host, field, state, graphData, cardList) {
    ...
    // …and that goes for the measures themselves. This band mounts at
    // `beforeBody`, above the section's own title, so on a first visit the
    // opening line of the whole field was `0% explored · 0% connected · 0/263
    // pages` — three zeros introducing a wiki the reader has not been given a
    // reason to read yet. They are a record of progress and they belong here
    // the moment there is any. Until then the invitation below stands alone.
    if (started) {
    ...
    const list = started ? cov.frontier : []
    if (!started) {
      const line = el("p", { class: "pb-next", style: CASE })
      line.appendChild(document.createTextNode("Start learning: "))
      line.appendChild(
        el("a", {
          href: base() + "/learn?field=" + encodeURIComponent(field),
          text: "choose an ordered path",
          style: CASE,
        }),
      )
      ...
      const line = el("p", { class: "pb-next", style: CASE })
      line.appendChild(document.createTextNode("Next, from where you are: "))
      line.appendChild(links(list, 3))
      host.appendChild(line)
    }
    const map = el("p", { class: "pb-map" })
    map.appendChild(
      el("a", { href: mapLink(field), text: "View this field on the map →", style: CASE }),
    )
  ...
  function renderMap(host, graphData, state) {
    ...
    // Nothing read in what is on screen: every circle is the same colour, there
    // is no route and no frontier ring, and the map is a few hundred identical
    // dots with no way in. The legend below explains the three colours it is not
    // yet showing. Name the way in instead — the best-connected pages of each
    // field on screen, which is what a reader with no history should open first,
    // and the same question the hub band answers with `Start learning`.
    const anyRead = shown.some((node) => nodeTier(node.s, state) >= TIERS.read)
    if (!anyRead) {
      const start = []
      frontierFields.forEach((name) => start.push(...entryPoints(graphData, name).slice(0, 3)))
      start.sort((a, b) => b.degree - a.degree || a.slug.localeCompare(b.slug))
      if (start.length) {
        const line = el("p", { class: "pm-empty", style: CASE })
        line.appendChild(
          document.createTextNode(
            "Nothing read here yet — the map fills in as you read. The best-connected pages are a good way in: ",
          ),
        )
        line.appendChild(links(start, 3))
        host.appendChild(line)
      }
    }
    ...
    edgePaths.forEach((tier) =>
      drawing.appendChild(svgEl("path", { class: `pm-edge pm-edge-${tier}` })),
    )
    const edgeData = ["", "", ""]
    ...
    edgePaths.forEach((tier, i) =>
      drawing.querySelector(`.pm-edge-${tier}`).setAttribute("d", edgeData[i]),
    )
    ...
    const recent = (state.trail ?? []).filter(
      (entry) => Date.now() - (entry.at ?? 0) <= TRAIL_WINDOW,
    )
    ...
    for (const node of shown) {
      ...
      if (frontier.has(node.s)) {
        drawing.appendChild(
          svgEl("circle", {
            ...
            cx: p.x,
            cy: p.y,
            r: Math.max(10, Math.min(16, 8 + Math.sqrt(node.d || 1) * 1.7)),
          }),
        )
      }
      const a = svgEl("a", {
      ...
      const circle = svgEl("circle", {
        class: `pm-node pm-node-${tier}`,
        cx: p.x,
        cy: p.y,
        r: Math.max(5, Math.min(11, 4 + Math.sqrt(node.d || 1) * 1.7)),
        tabindex: "0",
      })
    ...
    ;[
      ...
      ["pm-node-1", "seen"],
      ["pm-node-2", "read"],
      // The ring is drawn whenever anything has been read, and it is the mark a
      // reader most needs named — it is the only one that points forward.
      ["pm-key-frontier", "next"],
    ].forEach(([mark, name]) => {
      legend.appendChild(el("span", { class: `pm-key ${mark}` }))
    ...
    table.innerHTML =
      "<thead><tr><th>field</th><th>explored</th><th>connected</th><th>pages</th></tr></thead>"
    const body = el("tbody")
    names.forEach((name) => {
      const cov = coverage(graphData, name, (slug) => nodeTier(slug, state) >= TIERS.read)
      const row = el("tr")
  ...
  window.addEventListener("symposium:cloud-merged", () => {
    band()
    map()
  })
