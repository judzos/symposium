(function learningModeRuntime() {
  ...
  function cards() {
    if (!indexPromise) {
      ...
      indexPromise = fetch(base() + "/static/cards.json?schema=2", { cache: "no-store" })
  ...
  function pageTitles(index) {
    ...
    for (const card of index.cards ?? []) {
      if (!titles.has(card.slug)) titles.set(card.slug, card.title)
    }
    return titles
  }
  ...
  function syllabusCard(track, readPages, titles) {
    ...
    return el(
      ...
      el("span", {
        class: "lm-track-meta",
        text: track.level + " · " + done + " of " + total + " lessons read",
      }),
      ...
      for (const track of tracks.filter((row) => row.section === field)) {
        ...
        grid.appendChild(
          el(
            ...
            el("span", {
              class: "lm-track-meta",
              text: track.level + " · " + total + " lessons · " + track.cards + " cards",
            }),
            el("h3", { text: track.title }),
            el("p", { text: track.description }),
            el(
